...
class PickActionClass(object):
    ...
    def as_cb(self, goal):
        ...
        for point in cycle(points):  # initial code
            head_control.run(point)
            if self._aruco_marker is not None: break

	# Tiago looks 1 loop around before picking:
	#it = 0  # added by myself
        #for point in cycle(points):
        #    head_control.run(point)
	#    it += 1  # added by myself
	#    if it > 5:  # added by myself
        #    	if self._aruco_marker is not None: break
        ...
        # 4. Go to grasp position
 	pre_goal = PoseStamped()
        pre_goal.pose = res.grasp_pose
	pre_goal.pose.position.z += 0.1
	pre_goal.pose.position.y -= 0.12
